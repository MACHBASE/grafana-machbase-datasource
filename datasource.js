...
exports.MachbaseDatasource = void 0;
...
var MachbaseDatasource =
/*#__PURE__*/
function () {
  function MachbaseDatasource(instanceSettings, $q, backendSrv, templateSrv) {
    _classCallCheck(this, MachbaseDatasource);
  ...
  _createClass(MachbaseDatasource, [{
    key: "sqlQuery",
    value: function sqlQuery(aSql, aLimit) {
      var query = {
        'query': aSql,
        'limit': aLimit
      };
      var sRet = this.backendSrv.datasourceRequest({
        url: "".concat(this.url, "/query/grafana4sql"),
        data: query,
        method: 'POST',
        headers: {
    ...
    value: function query(options) {
      ...
      return this.backendSrv.datasourceRequest({
        url: "".concat(this.url, "/query/grafana4"),
        data: query,
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        ...
        if (response.config.data.pluginid === 'machgraph' || response.config.data.pluginid === 'graph') {
          ...
          for (var i = 0; i < response.data.length; i++) {
            ...
            if (sAlarm.length > 0) {
              ...
              for (var j = 0; j < sAlarm.length; j++) {
                ...
                if (!sRes) continue;
                var sErrorMsg = sReducer + (sRange === 'all' ? '() of all datas' : "() of datas in ".concat(sRange)) + " (".concat(sValue.toString(), ") ").concat(sTempEvalType[sEvaluator.type]);
                ...
                switch (sEvaluator.type) {
                  ...
                  case 'out':
                  case 'in':
                    sErrorMsg += "".concat(Number(sEvaluator.value1).toString(), " to ").concat(Number(sEvaluator.value2).toString());
                    break;
                }
                ...
                if (sType === 'each') {
                  ...
                  if (sRes) {
                    ...
                    sGlobalCond += "[".concat(response.data[i].target, "] ").concat(sErrorMsg);
                    sGlobalTitle += "'".concat(response.data[i].target, "'");
                  }
                }
              }
              ...
              if (sAlert !== null && sAlert) {
                ...
                sMsg += "Graph Title : ".concat(response.data[i].target, "\n");
                sMsg += "Duration : ".concat(TmpDate1.toLocaleString(), " - ").concat(TmpDate2.toLocaleString(), "\n");
                sMsg += "Alert Time : ".concat(TmpDate0.toLocaleString(), "\n");
                sMsg += "Condition : ".concat(sErroredCond, "\n");

                _this.backendSrv.alertSrv.set("Alert in '".concat(response.data[i].target, "'"), sErroredCond, 'error', 7000);

                var sUrl = "".concat(_this.url, "/sendmail");
                var sOptions = {
                  subject: "Alert in '".concat(response.data[i].target, "'"),
                  message: sMsg,
          ...
          if (sGolbalAlert !== null && sGolbalAlert) {
            ...
            var TmpDate2 = new Date(sEndTime);
            sMsg = 'Alert information.\n';
            sMsg += "Graph Title : ".concat(sGlobalTitle, "\n");
            sMsg += "Duration : ".concat(TmpDate1.toLocaleString(), " - ").concat(TmpDate2.toLocaleString(), "\n");
            sMsg += "Alert Time : ".concat(TmpDate0.toLocaleString(), "\n");
            sMsg += "Condition : ".concat(sGlobalCond, "\n");

            _this.backendSrv.alertSrv.set("Alert in ".concat(sGlobalTitle), sGlobalCond, 'error', 7000);

            var sUrl = "".concat(_this.url, "/sendmail");
            var sOptions = {
              subject: "Alert in ".concat(sGlobalTitle),
              message: sMsg,
              range_begin: TmpDate1.toString(),
              range_end: TmpDate2.toString(),
              when: TmpDate0.toString(),
    ...
    value: function metricFindQuery(aQuery) {
      ...
      return this.backendSrv.datasourceRequest({
        url: "".concat(this.url, "/query/grafana4metric"),
        data: {
          'query': sQuery
        },
        method: 'POST',
    ...
    value: function buildQueryParameters(options, aVariables) {
      var _this = this; //remove empty targets


      options.targets = _lodash["default"].filter(options.targets, function (target) {
        return target.hasOwnProperty('target') && target.target.hasOwnProperty('metric') && _lodash["default"].isEmpty(target.target.errors) && (target.target.rawQuery || target.target.metric !== '' && target.target.column !== '') && (!target.target.rawQuery || target.target.query !== '');
      ...
      var targets = _lodash["default"].map(options.targets, function (target) {
        var sFilters = _lodash["default"].map(target.target.filters, function (filter) {
          var sKey = filter.key;
        ...
        sFilters = _lodash["default"].filter(sFilters, function (f) {
          return f.key !== '--select column--' && f.value !== '';
        });
    ...
    key: "MakeReplaceVariables",
    value: function MakeReplaceVariables(aVariables, aOptions, sRegs, sVals) {
      var _this = this;
      ...
      if (sUseOption) {
        ...
        if (this.templateSrv.hasOwnProperty('index') && this.templateSrv.index.hasOwnProperty('__from')) {
          ...
          // below ver 6.0($__from & $__to are supported from ver 6.0)
      ...
      var sVariables = _lodash["default"].filter(aVariables, function (aVar) {
        if (aVar.type !== 'datasource') {
          ...
          if (typeof sVal === 'undefined') {
          ...
          } else if (sVal === "$__auto_interval_".concat(aVar.name)) {
            ...
            } else if (sTemp % 60000 === 0) {
              // min (60,000ms)
              sVal = parseInt(sTemp / 60000).toString() + 'm';
            } else if (sTemp % 1000 === 0) {
              // sec (1,000ms)
              sVal = parseInt(sTemp / 1000).toString() + 's';
            } else if (sTemp < 3000) {
              // ms 3,000ms = 3sec
              sVal = sTemp.toString() + 'ms';
            } else if (sTemp <= 180000) {
              // sec (180,000ms = 3min)
              sVal = parseInt(sTemp / 1000).toString() + 's';
            } else if (sTemp <= 10800000) {
              // min (10,800,000ms = 3hour)
              sVal = parseInt(sTemp / 60000).toString() + 'm';
            } else if (sTemp <= 259200000) {
              // hour (259,200,000ms = 3day)
              sVal = parseInt(sTemp / 3600000).toString() + 'h';
            } else {
              // day
  ...
  return MachbaseDatasource;
}();

exports.MachbaseDatasource = MachbaseDatasource;

MachbaseDatasource.prototype.convertToMachbaseInterval = function (intervalString) {
  ...
  var value = matches[1];
  var unit = matches[2];
  var ms = Math.round(_kbn["default"].intervals_in_seconds[unit] * parseInt(value) * 1000);
...
MachbaseDatasource.prototype.calcAlertValue = function (aReducer, aFrom, aDatas) {
  var sReducer = aReducer;
  var sFrom = aFrom;
  var sDatas = aDatas;
  var sValue = null;

  if (sReducer === 'median') {// reserved
  } else {
...
MachbaseDatasource.prototype.EvalAlert = function (aValue, aEvaluator) {
  var sValue = aValue;
  ...
  switch (sType) {
    // is above(gt), is below(lt), is outside range{out}, is within range{in}, has no value{no}
    case 'gt':
